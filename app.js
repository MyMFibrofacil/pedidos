...
const html = {
  ...
  summaryLabelPrimary: document.getElementById("summary-label-primary"),
  summaryLabelSecondary: document.getElementById("summary-label-secondary"),
  summaryByThickness: document.getElementById("summary-by-thickness"),
  summaryByThicknessSecondary: document.getElementById("summary-by-thickness-secondary"),
...
function applyClientUi() {
  ...
  const labels = getThicknessEntries();
  if (html.summaryLabelPrimary && labels[0]) {
    html.summaryLabelPrimary.textContent = `Total ${labels[0].summaryLabel}:`;
  }
  if (html.summaryLabelSecondary && labels[1]) {
    html.summaryLabelSecondary.textContent = `Total ${labels[1].summaryLabel}:`;
  }
}
...
function renderIndividualProductRow(family, product) {
...
function renderFamilyCard(family) {
  ...
  const products =
    family.type === "grupo" && searchTerm ? family.products : filteredProducts;
      ...
      ${
        isOpen
          ? family.type === "grupo"
            ...
            : `<div class="divide-y divide-slate-100">${products
                .map((product) => renderIndividualProductRow(family, product))
                .join("")}</div>`
          : ""
      }
    </section>
...
function renderSummary() {
  const data = summary();
  html.summaryByThickness.textContent = `${data.totalsByThickness["3"] || 0} placas`;
  html.summaryByThicknessSecondary.textContent = `${data.totalsByThickness["15"] || 0} placas`;
  html.sendButton.disabled = data.totalPlates === 0;

  if (data.sections.length === 0) {
    html.summaryDetailsList.innerHTML =
      '<p class="p-4 text-sm text-slate-500">Todavía no agregaste placas al pedido.</p>';
    summaryOpen = false;
  } else {
                  ...
                  return `
                    <div class="p-4 space-y-2">
                      <div class="flex items-start justify-between gap-3">
                        ...
                        <p class="text-sm font-bold text-primary shrink-0">${family.totalPlates} placas</p>
                      </div>
                      <div class="space-y-1">
                        ${family.breakdown
                          .map((item) => {
...
function buildWhatsAppText() {
  ...
  lines.push("Hola, comparto el siguiente pedido:");
  ...
  data.sections.forEach((section) => {
    ...
    lines.push(`*Total ${sectionMeta.summaryLabel}: ${section.totalPlates} placas*`);
  });
...
async function sendToWhatsApp() {
  ...
  if (copied) {
    setStatus("Pedido copiado. Elegí el grupo en WhatsApp y enviá el mensaje.", "success");
  } else {
    setStatus("WhatsApp se abrió con el pedido. Si no aparece el texto, copiá el detalle desde el resumen.", "error");
  }
}
...
async function loadCatalogFromSheet() {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    const normalizedFamilyName =
      normalizedType === "individual" ? "Individuales" : familyName;
...
async function init() {
  ...
  try {
    ...
    if (!catalog.length) {
      setFamiliesMessage("No hay esquemas disponibles en la hoja.");
